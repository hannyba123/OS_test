...
#'mem':'vsish -e get /memory/comprehensive',
'disk':'esxcli storage core adapter list',
#'disk':'df -H|sort|grep -Ev "tmpfs|mapper"',
#'disk':'lsblk -t',
'nic':'esxcli network nic list'}
list_ = ['cpu','disk','nic']
mesg_ = 'dmesg -T |egrep -i "error|failed|warning|bug|respond|wrong|pending|call trace"|grep -v cpu'
...
def connect_sut():
        args = True
        ssh = SshConnect(os_ip_ad,username,passwd,PORT)
        while args:
                try:
                        time.sleep(0.5)
...
#	execmd = 'dmesg -T |egrep -i "error|failed|warning|bug|respond|wrong|pending|call trace"|egrep -v cpu'
	execmd = mesg_
	stdin,stdout,stderr = ssh_.exec_command(execmd)
	if times_ == 1:
...
def sel_check(ssh_,times_):
        execmd = '/opt/ipmitool/bin/ipmitool sel elist'
        stdin,stdout,stderr = ssh_.exec_command(execmd)
...
def sel_clear(ssh_):
        execmd = '/opt/ipmitool/bin/ipmitool sel clear'
        stdin,stdout,stderr = ssh_.exec_command(execmd)

def restart(ssh_ , times,which):
        if which == 'reboot':
                execmd = 'date && reboot'
                stdin,stdout,stderr = ssh_.exec_command (execmd)
...
if __name__ == "__main__":
    sure = input('Are you sure SUT install ipmitool? y or n\n').lower()
    if sure == 'y':
#         global logger
